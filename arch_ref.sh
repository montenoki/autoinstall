...
check_partition_to_make() {
	...
	while [ $list_num -lt ${#check_list[@]} ]; do
...
find_new_disk() {
	...
	findcount1='0'
	findcount2='0'
	while [ $findcount1 -lt ${#new_table[@]} ]; do
		while [ $findcount2 -lt ${#PROTECT_DISK[@]} ]; do
...
diskpart() {
	...
	while [ $disk_num -lt ${#PARTITION_TO_MAKE[@]} ]; do
